...
export async function logAccess(page) {
  try {
    ...
    if (error) {
      // Silenciosamente ignora erro se tabela não existir (42P01)
      if (error.code === '42P01' || 
          (error.message && error.message.includes('does not exist'))) {
        console.warn('[logAccess] Tabela access_logs não existe. Crie no Supabase: CREATE TABLE access_logs (id uuid DEFAULT gen_random_uuid(), user_id uuid REFERENCES auth.users(id), page text, accessed_at timestamptz, created_at timestamptz);');
      ...
      if (error.code === '42501' || 
        ...
        console.warn('[logAccess] Permissão negada (RLS). Adicione policy para access_logs.');
        return { success: false, reason: 'permission_denied' };
      }

      // Outros erros - loga mas não quebra
      if (!error.message.includes('does not exist') && error.code !== '42P01') {
        console.warn('[logAccess] Erro ao registrar acesso:', error.message);
      }
    ...
    // Silenciosamente ignora qualquer erro inesperado
...
export async function checkSingleSession() {
  ...
  if (serverSession && storedSession && serverSession !== storedSession) {
    await signOut();
    localStorage.removeItem('session_id');
    alert('?? Sua sessao foi encerrada porque voce fez login em outro dispositivo.');
    window.location.href = 'index.html';
    return false;
  }
  return true;
